...
export function OrderCard({ order, onCall, onComplete, isLoading }: OrderCardProps) {
    ...
    // Determine card border based on urgency
    let borderClass = '';
    if (minutesWaiting > 30 && order.status !== 'PICKED_UP') {
        borderClass = 'border-l-4 border-l-danger-500';
    }

    return (
        <div className={`card-premium p-4 space-y-3 ${borderClass}`}>
            {/* Header */}
            <div className="flex items-start justify-between">
                <div className="flex-1">
                    <div className="flex items-center gap-2">
                        <h3 className="text-lg font-bold text-dark-900">#{order.orderCode}</h3>
                        {order.whatsappSent && (
                    ...
                    <p className="text-sm text-dark-600 mt-1">{order.customerName}</p>
                </div>
                <div className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statusInfo.color}`}>
                    {statusInfo.label}
                </div>
            </div>
            ...
            <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className={minutesWaiting > 30 ? 'text-danger-700 font-bold' : 'text-dark-700'}>
                        {minutesWaiting} min aguardando
                    </span>
                </div>
                <Progress
                    value={progressValue}
