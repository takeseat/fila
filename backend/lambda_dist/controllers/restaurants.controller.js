"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RestaurantsController = void 0;
const database_1 = __importDefault(require("../config/database"));
const zod_1 = require("zod");
const updateSettingsSchema = zod_1.z.object({
    waitingAlertMinutes: zod_1.z.number().int().min(1).nullable().optional(),
    calledAlertMinutes: zod_1.z.number().int().min(1).nullable().optional(),
    avgWaitWindowMinutes: zod_1.z.number().int().min(1).nullable().optional(),
    avgWaitFallbackMinutes: zod_1.z.number().int().min(1).nullable().optional(),
});
const updateBusinessDataSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(255),
    tradeName: zod_1.z.string().max(255).nullable().optional(),
    phone: zod_1.z.string().min(1),
    countryCode: zod_1.z.string().length(2), // ISO 3166-1 alpha-2
    stateCode: zod_1.z.string().nullable().optional(),
    city: zod_1.z.string().min(1),
    addressLine: zod_1.z.string().nullable().optional(),
    addressNumber: zod_1.z.string().nullable().optional(),
    addressComplement: zod_1.z.string().nullable().optional(),
    postalCode: zod_1.z.string().nullable().optional(),
    // email and cnpj are READ-ONLY, not accepted in updates
});
class RestaurantsController {
    /**
     * Get business data (name, address, contact)
     */
    async getBusinessData(req, res) {
        try {
            const restaurantId = req.user.restaurantId;
            const restaurant = await database_1.default.restaurant.findUnique({
                where: { id: restaurantId },
                select: {
                    name: true,
                    tradeName: true,
                    email: true,
                    cnpj: true,
                    phone: true,
                    countryCode: true,
                    stateCode: true,
                    city: true,
                    addressLine: true,
                    addressNumber: true,
                    addressComplement: true,
                    postalCode: true,
                }
            });
            if (!restaurant) {
                res.status(404).json({ error: 'Restaurant not found' });
                return;
            }
            res.json(restaurant);
        }
        catch (error) {
            console.error('Error fetching business data:', error);
            res.status(500).json({ error: 'Internal server error' });
        }
    }
    /**
     * Update business data (editable fields only)
     */
    async updateBusinessData(req, res) {
        try {
            const restaurantId = req.user.restaurantId;
            const data = updateBusinessDataSchema.parse(req.body);
            const updatedRestaurant = await database_1.default.restaurant.update({
                where: { id: restaurantId },
                data: {
                    name: data.name,
                    tradeName: data.tradeName,
                    phone: data.phone,
                    countryCode: data.countryCode,
                    stateCode: data.stateCode,
                    city: data.city,
                    addressLine: data.addressLine,
                    addressNumber: data.addressNumber,
                    addressComplement: data.addressComplement,
                    postalCode: data.postalCode,
                    // email and cnpj are intentionally NOT updated
                },
                select: {
                    name: true,
                    tradeName: true,
                    email: true,
                    cnpj: true,
                    phone: true,
                    countryCode: true,
                    stateCode: true,
                    city: true,
                    addressLine: true,
                    addressNumber: true,
                    addressComplement: true,
                    postalCode: true,
                }
            });
            res.json(updatedRestaurant);
        }
        catch (error) {
            if (error instanceof zod_1.z.ZodError) {
                res.status(400).json({ error: 'Validation error', details: error.errors });
                return;
            }
            console.error('Error updating business data:', error);
            res.status(500).json({ error: 'Internal server error' });
        }
    }
    /**
     * Get restaurant settings
     */
    async getSettings(req, res) {
        try {
            const restaurantId = req.user.restaurantId;
            const restaurant = await database_1.default.restaurant.findUnique({
                where: { id: restaurantId },
                select: {
                    waitingAlertMinutes: true,
                    calledAlertMinutes: true,
                    avgWaitWindowMinutes: true,
                    avgWaitFallbackMinutes: true,
                    name: true,
                    email: true,
                    phone: true,
                    timezone: true
                }
            });
            if (!restaurant) {
                res.status(404).json({ error: 'Restaurant not found' });
                return;
            }
            res.json(restaurant);
        }
        catch (error) {
            console.error('Error fetching settings:', error);
            res.status(500).json({ error: 'Internal server error' });
        }
    }
    /**
     * Update restaurant settings
     */
    async updateSettings(req, res) {
        try {
            const restaurantId = req.user.restaurantId;
            const data = updateSettingsSchema.parse(req.body);
            const updatedRestaurant = await database_1.default.restaurant.update({
                where: { id: restaurantId },
                data: {
                    waitingAlertMinutes: data.waitingAlertMinutes,
                    calledAlertMinutes: data.calledAlertMinutes,
                    avgWaitWindowMinutes: data.avgWaitWindowMinutes,
                    avgWaitFallbackMinutes: data.avgWaitFallbackMinutes,
                },
                select: {
                    waitingAlertMinutes: true,
                    calledAlertMinutes: true,
                    avgWaitWindowMinutes: true,
                    avgWaitFallbackMinutes: true,
                }
            });
            res.json(updatedRestaurant);
        }
        catch (error) {
            if (error instanceof zod_1.z.ZodError) {
                res.status(400).json({ error: 'Validation error', details: error.errors });
                return;
            }
            console.error('Error updating settings:', error);
            res.status(500).json({ error: 'Internal server error' });
        }
    }
}
exports.RestaurantsController = RestaurantsController;
//# sourceMappingURL=restaurants.controller.js.map