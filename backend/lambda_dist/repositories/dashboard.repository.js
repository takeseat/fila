...
class DashboardRepository {
     ...
     */
    async getActiveQueueCount(restaurantId) {
        const result = await database_1.default.$queryRawUnsafe(`SELECT COUNT(*) as count
             FROM waitlist_entries
             WHERE restaurant_id = ?
               AND status IN ('WAITING', 'CALLED')`, restaurantId);
        return Number(result[0].count);
    }
    /**
     * Get count of customers seated today
     */
    async getSeatedTodayCount(restaurantId) {
        const result = await database_1.default.$queryRawUnsafe(`SELECT COUNT(*) as count
             FROM waitlist_entries
             WHERE restaurant_id = ?
               AND status = 'SEATED'
               AND DATE(seated_at) = CURDATE()`, restaurantId);
        return Number(result[0].count);
    }
    /**
     * Get P90 (90th percentile) wait time for customers seated today (in minutes)
     */
    async getAverageWaitTimeToday(restaurantId) {
        const result = await database_1.default.$queryRawUnsafe(`WITH wait_times AS (
                SELECT 
     ...
     */
    async getCancelledTodayCount(restaurantId) {
        const result = await database_1.default.$queryRawUnsafe(`SELECT COUNT(*) as count
             FROM waitlist_entries
             WHERE restaurant_id = ?
               AND (
     ...
     */
    async getHourlyVolumeToday(restaurantId) {
        const result = await database_1.default.$queryRawUnsafe(`SELECT 
     ...
     */
    async getDailyVolumeLast7Days(restaurantId) {
        const result = await database_1.default.$queryRawUnsafe(`SELECT 
     ...
     */
    async getYesterdayMetrics(restaurantId) {
        // Active queue count at same time yesterday
        const activeQueueResult = await database_1.default.$queryRawUnsafe(`SELECT COUNT(*) as count
             FROM waitlist_entries
             WHERE restaurant_id = ?
               AND status IN ('WAITING', 'CALLED')
               AND DATE(created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
               AND HOUR(created_at) = HOUR(NOW())`, restaurantId);
        // Seated count yesterday
        const seatedResult = await database_1.default.$queryRawUnsafe(`SELECT COUNT(*) as count
             FROM waitlist_entries
             WHERE restaurant_id = ?
               AND status = 'SEATED'
               AND DATE(seated_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)`, restaurantId);
        // P90 wait time for yesterday
        const p90Result = await database_1.default.$queryRawUnsafe(`WITH wait_times AS (
                SELECT 
                    TIMESTAMPDIFF(MINUTE, created_at, seated_at) as wait_minutes,
                    ROW_NUMBER() OVER (ORDER BY TIMESTAMPDIFF(MINUTE, created_at, seated_at)) as row_num,
                    COUNT(*) OVER () as total_count
